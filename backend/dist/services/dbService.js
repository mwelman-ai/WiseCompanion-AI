...
const initMockData = () => {
    ...
    mockHealthLogs.push({ id: 'h-1', userId: defaultUser.id, logType: 'walking', value: 25, createdAt: `${todayStr}T09:30:00.000Z` }, { id: 'h-2', userId: defaultUser.id, logType: 'water', value: 250, createdAt: `${todayStr}T10:00:00.000Z` }, { id: 'h-3', userId: defaultUser.id, logType: 'water', value: 250, createdAt: `${todayStr}T14:15:00.000Z` }, { id: 'h-4', userId: defaultUser.id, logType: 'weight', value: 68.5, createdAt: `${todayStr}T08:00:00.000Z` });
    // Medications
...
export const dbService = {
    ...
    async updateUserPlan(userId, planTier) {
        if (isMockMode) {
    ...
    async updateMedication(medId, userId, updates) {
        if (isMockMode) {
            const idx = mockMedications.findIndex(m => m.id === medId && m.userId === userId);
            if (idx === -1)
                return null;
            mockMedications[idx] = { ...mockMedications[idx], ...updates };
    ...
    async deleteMedication(medId, userId) {
        if (isMockMode) {
            const idx = mockMedications.findIndex(m => m.id === medId && m.userId === userId);
            if (idx === -1)
                return false;
    ...
    async updateTravelPlan(planId, userId, updates) {
        if (isMockMode) {
            const idx = mockTravelPlans.findIndex(t => t.id === planId && t.userId === userId);
            if (idx === -1)
                return null;
            mockTravelPlans[idx] = { ...mockTravelPlans[idx], ...updates };
    ...
    async updateEmergencyContact(contactId, userId, updates) {
        if (isMockMode) {
            const idx = mockContacts.findIndex(c => c.id === contactId && c.userId === userId);
            if (idx === -1)
                return null;
            mockContacts[idx] = { ...mockContacts[idx], ...updates };
