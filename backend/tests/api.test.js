...
describe('WiseCompanion AI - Backend Integration Tests', () => {
    ...
    test('POST /scam-check - should flag likely scams correctly', async () => {
        ...
        const res = await axios.post(`${API_URL}/scam-check`, { text: scamText }, { headers: { 'Authorization': 'Bearer mock-user-123' } });
        assert.strictEqual(res.status, 200);
    ...
    test('POST /scam-check - should mark safe text correctly', async () => {
        ...
        const res = await axios.post(`${API_URL}/scam-check`, { text: safeText }, { headers: { 'Authorization': 'Bearer mock-user-123' } });
        assert.strictEqual(res.status, 200);
    ...
    test('POST /scam/detect - should support the frontend path and work identically', async () => {
        const scamText = 'URGENT: Your bank account is suspended. Click tinyurl.com/123 to verify now!';
        const res = await axios.post(`${API_URL}/scam/detect`, { text: scamText }, { headers: { 'Authorization': 'Bearer mock-user-123' } });
        assert.strictEqual(res.status, 200);
        assert.strictEqual(res.data.status, 'dangerous');
        assert.ok(res.data.probability > 50);
    });
    ...
    test('POST /chat - should return warm companion answers', async () => {
        const res = await axios.post(`${API_URL}/chat`, { prompt: 'Hello, I feel a bit lonely today.' }, { headers: { 'Authorization': 'Bearer mock-user-123' } });
        assert.strictEqual(res.status, 200);
    ...
    test('POST /health/log and GET /health/logs', async () => {
        // 1. Post a health log
        const logRes = await axios.post(`${API_URL}/health/log`, { logType: 'walking', value: 6500 }, { headers: { 'Authorization': 'Bearer mock-user-123' } });
        ...
        // 2. Retrieve logs
        const retrieveRes = await axios.get(`${API_URL}/health/logs`, { headers: { 'Authorization': 'Bearer mock-user-123' } });
        assert.strictEqual(retrieveRes.status, 200);
        assert.ok(Array.isArray(retrieveRes.data.logs));
        const hasOurLog = retrieveRes.data.logs.some((l) => l.value === 6500 && l.logType === 'walking');
        assert.ok(hasOurLog);
    });
});
